...
OPTIONS (
  format = 'CSV',
  uris = ['gs://source_layer/trans.csv'],
  skip_leading_rows = 1
);
...
OPTIONS (
  format = 'CSV',
  uris = ['gs://source_layer/account.csv'],
  skip_leading_rows = 1
);
...
OPTIONS (
  format = 'CSV',
  uris = ['gs://source_layer/client.csv'],
  skip_leading_rows = 1
);
...
OPTIONS (
  format = 'CSV',
  uris = ['gs://source_layer/district.csv'],
  skip_leading_rows = 1
);
...
OPTIONS (
  format = 'CSV',
  uris = ['gs://source_layer/order.csv'],
  skip_leading_rows = 1
);
